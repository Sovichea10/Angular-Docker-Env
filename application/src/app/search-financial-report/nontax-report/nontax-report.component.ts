...
@Component({
    selector: 'nontax-report',
    templateUrl: './nontax-report.component.html',
    styleUrls: ['./nontax-report.component.scss'],
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations
})
export class NonTaxReportComponent implements OnInit {
    ...
    downloadExcel(url){
        this.dialog.open(LoadingDialogComponent, { disableClose: true });
        this.service.getNonTaxReportExcel({'url': url, 'mobile':this.isMobile, 'tem_token':this.tem_token}, this.token).subscribe( res => {
    ...
    //    if(this.from || this.to){
        ...
        let new_url = '';
        if(this.date_cycle == '1'){
            this.pdfUrl = 'level='+this.level+'&profit_center='+this.profit_center+'&public_service_group_code='+this.public_service_group_code+'&date_cycle='+this.date_cycle+'&from='+this.year_from+'&to='+this.year_to+'&system='+this.system;
        }
        if(this.date_cycle == '2'){
            this.pdfUrl = 'level='+this.level+'&profit_center='+this.profit_center+'&public_service_group_code='+this.public_service_group_code+'&date_cycle='+this.date_cycle+'&from='+this.month_from+'&to='+this.month_to+'&system='+this.system;
        }
        if(this.date_cycle == '3'){
            this.pdfUrl = 'level='+this.level+'&profit_center='+this.profit_center+'&public_service_group_code='+this.public_service_group_code+'&date_cycle='+this.date_cycle+'&from='+from+'&to='+to+'&system='+this.system;
        }
