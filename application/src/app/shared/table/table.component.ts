...
export class TableComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.matDataSource.filterPredicate = (data:any,filter:any) =>{
      const dataStr = JSON.stringify(data).toLowerCase();
      return dataStr.indexOf(filter) != -1;
    }

    /**
     * to search record in table
     * @param _commonService subscribe callout from search record
     */
    this._commonService.TextcallOut.subscribe((searchValue:any)=>{
        this.matDataSource.filter = searchValue.trim().toLowerCase();    
    });
